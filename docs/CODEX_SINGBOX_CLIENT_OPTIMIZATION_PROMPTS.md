...
- 规则资产事务：运行时远端 OTA 已退役。每次启动/重载会在与更新器共用的跨进程锁下，把 APK 内置、SHA-256 锁定的两份 SRS 修复到私有目录，再复制为内容寻址、不可变快照；`Libbox.checkConfig`、独立 preflight 和 live start 全部使用同一个快照目录。损坏/混代源文件不能发布快照；损坏的已发布快照会保留给可能仍在运行的旧 core，并旁路生成可复用的修复代次。旧 WorkManager 任务只会被取消或本地校验，不能重新下载。
...
- 解析预算：订阅/二维码/深链会先限制 UTF-8 输入字节、物理行/候选数、单链接大小与 Base64/VMess 解码大小；外部 `sn://` 还限制 48 KiB 编码负载与 1 MiB zlib 解压输出，而受信任的本地导出兼容路径仍保留原有预算。失败不会先建库或删除旧节点。
- 启动与停止恢复：VPN 权限拒绝、前台服务/boot/Binder 恢复失败会持久化可行动提示；只有新 core 真正 Connected 才清除。显式停止会取消延迟选节点重连，状态机也禁止迟到 reconnect 覆盖 terminal stop；前台服务先完成 Android 所需的最小提升，再在后台验证连接意图，未确认时不创建 core/TUN。
...
| 已关闭（源码/原子测试，2026-07-26） | `DefaultNetworkListener.start(key)` 曾以覆盖式 map 表示 owner，且取消、销毁、迟到安装和已排队回调可能遗留或越过旧生命周期。 | 2、7 | `start()` 返回唯一 lease；取消补偿与 actor release fence 不遗留 callback；同步 `close()` 先失效同一 active token，`closeAndJoin()` 幂等等待最后 owner 的系统注销调用返回；BaseService、SagerNet 与 ServerLocation 使用代际/CAS 发布门。确定性交错测试已覆盖取消、销毁先于返回、排队事件、同 key 迟到 close、close/start、disable→enable、重复 close 与最后 lease 注销。仍需 arm64 真机网络切换、VPN/TUN 与真实 egress 验收。 |
...
| 已关闭（源码/原子测试，设备验证待完成，2026-07-26） | desired/applied 分应用 policy 的 revision/CAS、durable applied receipt 与 TUN generation 源码状态机已实现，陈旧草稿、迟到确认/回滚和失败恢复有原子测试；但 arm64 双测试 App 的真实出口仍未验证。 | 6、9 | 保留版本化 desired/applied 状态机、冲突保护、durable receipt 和失败恢复；补做绑定 APK SHA、package/UID、设备/API 与出口结果的 arm64 真机双测试 App 验收。在此之前不得把 UI、模拟器、APK 构建或单测写成真机 VPN/egress 通过。 |
