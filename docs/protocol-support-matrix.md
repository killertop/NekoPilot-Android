...
| Hysteria、Hysteria2、TUIC | URI + bean | 已实现 | `with_quic` | 仅编译 fixture | 未验证 | 迁移中；不得宣称可连接 |
...
| AnyTLS | bean / 私有序列化 | 已实现 | 已启用 | 仅编译 fixture | 未验证 | 迁移中 |
...
2. 为 Hysteria/Hysteria2/TUIC/AnyTLS 加入官方 schema fixture、错误输入 fixture、arm64
   `checkConfig` 和合法真实节点验收。
