...
abstract class SagerDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        internal val MIGRATION_5_6 = object : Migration(5, 6) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("ALTER TABLE `rules` ADD COLUMN `config` TEXT NOT NULL DEFAULT ''")
            }
        }
        ...
        // Version 8 temporarily removed the historical traffic columns. Preserve them for
        // version-7 users while keeping the current runtime free of traffic polling.
        ...
        /** Rebuilds v9 because v10 removes tx/rx and adds persisted presentation metadata. */
        internal val MIGRATION_9_10 = object : Migration(9, 10) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL(
                    "CREATE TABLE `proxy_entities_v10` (" +
                        "`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL," +
                        "`groupId` INTEGER NOT NULL," +
                        "`type` INTEGER NOT NULL," +
                        "`userOrder` INTEGER NOT NULL," +
                        ...
                        "`displayAddressCache` TEXT NOT NULL," +
                        "`displayTypeCache` TEXT NOT NULL," +
                        "`hasExplicitName` INTEGER NOT NULL," +
                ...
                database.execSQL(
                    "INSERT INTO `proxy_entities_v10` (" +
                        "`id`,`groupId`,`type`,`userOrder`,`status`,`ping`,`uuid`,`error`," +
                        "`displayNameCache`,`displayAddressCache`,`displayTypeCache`," +
                        "`hasExplicitName`,`configRevision`,`socksBean`,`httpBean`,`ssBean`," +
                        "`vmessBean`,`trojanBean`,`trojanGoBean`,`mieruBean`,`naiveBean`," +
                        "`hysteriaBean`,`tuicBean`,`sshBean`,`wgBean`,`shadowTLSBean`," +
                        "`anyTLSBean`,`chainBean`,`nekoBean`,`configBean`) " +
                        "SELECT `id`,`groupId`,`type`,`userOrder`,`status`,`ping`,`uuid`,`error`," +
                        "'', '', '', 0, 0, `socksBean`,`httpBean`,`ssBean`,`vmessBean`," +
                        "`trojanBean`,`trojanGoBean`,`mieruBean`,`naiveBean`,`hysteriaBean`," +
                        "`tuicBean`,`sshBean`,`wgBean`,`shadowTLSBean`,`anyTLSBean`,`chainBean`," +
                        "`nekoBean`,`configBean` FROM `proxy_entities`"
                )
                database.execSQL("DROP TABLE `proxy_entities`")
                database.execSQL("ALTER TABLE `proxy_entities_v10` RENAME TO `proxy_entities`")
                database.execSQL(
                    "CREATE INDEX IF NOT EXISTS `index_proxy_entities_groupId_userOrder` " +
                        "ON `proxy_entities` (`groupId`, `userOrder`)"
                )
