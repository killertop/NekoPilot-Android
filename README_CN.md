...
Android UI、生命周期与 Room 数据库保留在 Kotlin；Java 仅保留经审核的持久化模型兼容 ABI；
源码及安装包不再包含 Rust。完整边界见 [LANGUAGE_STRATEGY.md](LANGUAGE_STRATEGY.md)。
...
libcore/data_core.go     Go 订阅与延迟规划
