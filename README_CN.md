...
- 用于订阅、URL、规则完整性、日志和延迟决策的 Go 数据核心
- Go sing-box 集成
- sing-box 1.13.14 及提交到仓库的 Neko Android 集成

Android UI、生命周期与 Room 数据库保留在 Kotlin；代理核心使用官方 libbox AAR，源码及安装包
不再包含 Java、业务 Go 或 Rust。完整边界见 [LANGUAGE_STRATEGY.md](LANGUAGE_STRATEGY.md)。

## 环境要求

- Android API 23 或更高版本。
- JDK 17。
- Android SDK 35、Build Tools 35.0.1。
- 仅重建官方 libbox AAR 时需要 Go。

## 开发运行

准备官方 libbox 运行时和内置规则资源：
...
app/libs/libbox.aar      官方 sing-box libbox 运行时（构建产物）
...
- [语言归属与迁移边界](LANGUAGE_STRATEGY.md)
